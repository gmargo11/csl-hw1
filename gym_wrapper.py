...
class URRobotGym(Env):
    ...
    def step(self, action):
        ...
        reward = -1
        print(state)
        pos_des = [0.6, -0.15, 1.0]

        dist = ((state[0]-pos_des[0])**2 + (state[1]-pos_des[1])**2 + (state[2]-pos_des[2])**2)**0.5
        reward = -10 * dist
        if dist < 0.075:
            reward = 100
            done = True
            print("goal state reached in training!")
        elif dist > 0.4:
            done = True
            print("too far away, reset!")
